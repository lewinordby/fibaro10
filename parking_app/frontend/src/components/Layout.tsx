...
const navigation = [
  ...
  { to: "/parkeringer", label: "Parkeringer", icon: "parking" as const },
  { to: "/dagslinje", label: "Dagslinje", icon: "clock" as const },
  { to: "/kjoretoy", label: "Kjøretøy", icon: "apps" as const },
  { to: "/omrade", label: "Område", icon: "compare" as const },
  { to: "/prognose", label: "Prognose", icon: "trend" as const },
  { to: "/oppgjor", label: "Oppgjør", icon: "calendar" as const },
  { to: "/arsutvikling", label: "Årssammenligning", icon: "chart" as const },
  { to: "/tidspunkt", label: "Tidspunkt", icon: "clock" as const },
  { to: "/ukesnitt", label: "Ukesnitt", icon: "trend" as const },
  { to: "/bilstatistikk", label: "Bilstatistikk", icon: "chart" as const },
  { to: "/oppslag", label: "Oppslag", icon: "apps" as const },
];

const titles: Record<string, { title: string; description: string }> = {
  "/": { title: "Parkeringsoversikt", description: "Ukesutvikling, nøkkeltall og de siste parkeringene." },
  "/parkeringer": { title: "Parkeringer", description: "Alle parkeringer på valgt dag med kjøretøygrunnlag." },
  "/dagslinje": { title: "Dagslinje", description: "Belegg og parkeringsforløp gjennom dagen." },
  "/kjoretoy": { title: "Kjøretøy", description: "Søk, historikk og opplysninger om registrerte kjøretøy." },
  "/omrade": { title: "Område", description: "Geografisk fordeling av kjøretøy og omsetning." },
  "/prognose": { title: "Prognose", description: "Forventet parkeringsaktivitet mot faktisk utvikling." },
  "/oppgjor": { title: "Oppgjør", description: "Park Nordic-oppgjør kontrollert mot interne kildetall." },
  "/arsutvikling": { title: "Årssammenligning", description: "Akkumulert antall, beløp og parkeringstid gjennom året." },
  "/tidspunkt": { title: "Tidspunkt", description: "Parkeringstid og omsetning fordelt på ukedag og klokkeslett." },
  "/ukesnitt": { title: "Ukesnitt", description: "Utvikling i beløp og parkeringstid per parkering." },
  "/bilstatistikk": { title: "Bilstatistikk", description: "Mest brukte kjøretøy sortert på aktivitet og betaling." },
  "/oppslag": { title: "Oppslag", description: "Manglende navn og område samt tilhørende verktøy." },
};

function headingFor(pathname: string) {
  if (titles[pathname]) return titles[pathname];
  if (pathname.startsWith("/kjoretoy/")) return { title: "Kjøretøydetaljer", description: "Opplysninger og komplett parkeringshistorikk for kjøretøyet." };
  if (pathname.startsWith("/oppgjor/")) return { title: "Oppgjørsdetaljer", description: "Tolkede verdier og originalt oppgjørsbilag." };
  return titles["/"];
}
      ...
      <div ref={sidebar} id="sidebar" className={`flex lg:flex! flex-col absolute z-40 left-0 top-0 lg:static lg:left-auto lg:top-auto lg:translate-x-0 h-[100dvh] overflow-y-scroll lg:overflow-y-auto no-scrollbar w-64 lg:w-20 lg:sidebar-expanded:!w-64 2xl:w-64! shrink-0 bg-white dark:bg-gray-800 p-4 transition-all duration-200 ease-in-out rounded-r-2xl shadow-xs ${open ? "translate-x-0" : "-translate-x-64"}`}>
        <div className="flex justify-between mb-10 pr-3 sm:px-2">
          ...
          <AppLink to="/" className="flex items-center text-xl font-bold text-gray-800 dark:text-gray-100">
            <span className="lg:hidden lg:sidebar-expanded:block 2xl:block">Parkering</span>
            <MosaicIcon name="parking" className="text-sky-500 lg:block lg:sidebar-expanded:hidden 2xl:hidden" size={24} />
          </AppLink>
        </div>
          ...
          <div>
            ...
            <ul className="mt-3">
              {navigation.map((item) => {
                ...
                return (
                  <li className={`px-3 py-2 rounded-lg mb-0.5 last:mb-0 ${active ? "bg-sky-500/[0.12] dark:bg-sky-500/[0.24]" : ""}`} key={item.to}>
                    <AppLink className={`block truncate transition duration-150 ${active ? "text-gray-800 dark:text-gray-100" : "text-gray-800 dark:text-gray-100 hover:text-gray-900 dark:hover:text-white"}`} to={item.to}>
                      <div className="flex items-center"><MosaicIcon name={item.icon} className={active ? "text-sky-500" : "text-gray-400 dark:text-gray-500"} /><span className={labelClass}>{item.label}</span></div>
                    </AppLink>
                  </li>
                );
              })}
      ...
      <div className="px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 lg:border-b border-gray-200 dark:border-gray-700/60">
          <div className="flex items-center">
            ...
            <span className="hidden lg:block text-sm font-semibold text-gray-600 dark:text-gray-300">Parkering</span>
          </div>
