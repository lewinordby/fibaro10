...
APP_BUILD = os.getenv("REVENUE_APP_BUILD", DEFAULT_BUILD)
APP_COMMIT = os.getenv("REVENUE_APP_COMMIT", os.getenv("APP_COMMIT", "unknown"))
APP_STARTED_AT = os.getenv("REVENUE_APP_STARTED_AT", "runtime")
...
app = FastAPI(title="Lilletorget Omsetning", docs_url=None, redoc_url=None, lifespan=lifespan)
...
def forwarded_headers(request: Request, *, accept: str = "application/json") -> dict[str, str]:
    headers = {
        "Accept": accept,
        "User-Agent": "lilletorget-revenue-app/1",
    }
...
async def ready(request: Request) -> JSONResponse:
    ...
    try:
        ...
        return JSONResponse({"ok": False, "service": "revenue_app", "core": str(exc)}, status_code=503)
    return JSONResponse({"ok": True, "service": "revenue_app", "core": "ready"})
...
@app.get("/api/{core_path:path}")
async def proxy_core_api(core_path: str, request: Request) -> Response:
    normalized = core_path.strip("/").casefold()
    if normalized not in ALLOWED_CORE_GET_PATHS:
        raise HTTPException(status_code=404, detail="Endepunktet er ikke tilgjengelig i omsetningsappen")
...
<html lang="no"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="color-scheme" content="light dark"><title>Logg inn - Omsetning</title><link rel="stylesheet" href="{css_href}"></head>
...
<h1 class="mb-2 text-3xl font-bold text-gray-800 dark:text-gray-100">Omsetning</h1><p class="mb-6 text-sm text-gray-500 dark:text-gray-400">Bruk samme konto som i Fibaro10.</p>
