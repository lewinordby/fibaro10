...
SCENE_ID = int(os.environ.get("HC3_LIGHT_RUNNER_SCENE_ID", "362"))
EXECUTE_AFTER_UPSERT = os.environ.get("HC3_LIGHT_RUNNER_EXECUTE", "false").strip().lower() in {
    "1",
    "true",
    "yes",
    "ja",
...
def main():
    ...
    lua = (root / "scripts" / "hc3_light_runner_scene_362.lua").read_text(encoding="utf-8")
    backup_dir = root / "outputs" / "hc3_inventory" / "backups"
    backup_dir.mkdir(parents=True, exist_ok=True)
    stamp = time.strftime("%Y%m%d_%H%M%S")

    _, current = request(f"/api/scenes/{SCENE_ID}")
    (backup_dir / f"scene_{SCENE_ID}_before_light_runner_{stamp}.json").write_text(
