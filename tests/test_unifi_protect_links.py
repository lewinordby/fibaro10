...
class UnifiProtectLinkTests(unittest.TestCase):
    def test_parking_timelapse_url_uses_expected_window(self):
        ...
        url = unifi_protect.unifi_protect_parking_timelapse_url(target)

        self.assertIsNotNone(url)
        parsed = urlparse(url)
        params = parse_qs(parsed.query)
        target_ms = int(target.replace(tzinfo=unifi_protect.LOCAL_TZ).timestamp() * 1000)
        self.assertIn(unifi_protect.UNIFI_PROTECT_PARKING_CAMERA_ID, parsed.path)
    ...
    def test_parking_timelapse_url_can_use_one_minute_before(self):
        ...
        url = unifi_protect.unifi_protect_parking_timelapse_url(target, before_seconds=60)
    ...
    def test_parking_timelapse_url_can_use_fifteen_seconds_before(self):
        ...
        url = unifi_protect.unifi_protect_parking_timelapse_url(target, before_seconds=15)
