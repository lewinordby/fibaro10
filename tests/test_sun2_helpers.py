...
class Sun2HelperTests(unittest.TestCase):
    ...
    def test_room_identity_keeps_stable_id_but_uses_current_physical_room_and_bed(self):
        identity = sun2_room_identity("Rom 10")

        self.assertEqual(identity["room_id"], "rom-11")
        self.assertEqual(identity["physical_room_number"], 10)
        self.assertEqual(identity["display_room_number"], 10)
        self.assertEqual(identity["sun2_bed_id"], "649")
    ...
    def test_room_label_uses_source_name_when_present(self):
        self.assertEqual(sun2_room_label("rom-12", "Deluxe"), "Rom 11 - Deluxe")
