...
async def wait_for_latest_code(after: datetime | None = None) -> str:
    deadline = time.monotonic() + CODE_WAIT_SECONDS
    last_error: Exception | None = None
    while time.monotonic() < deadline:
        try:
            return await asyncio.to_thread(fetch_latest_code, after=after)
        except Exception as exc:
            last_error = exc
            await asyncio.sleep(5)
    if last_error:
        raise RuntimeError(f"{last_error} Ventet {CODE_WAIT_SECONDS} sekunder etter at EasyPark ble bedt om å sende kode.") from last_error
    raise RuntimeError(f"Fant ingen EasyPark-kode innen {CODE_WAIT_SECONDS} sekunder.")
...
async def ensure_logged_in(page) -> bool:
    ...
    if re.search(r"enter code|verification code|code|kode", body, re.I):
        code = await wait_for_latest_code(after=requested_at)
...
async def run_download_import(
    ...
    try:
        return await asyncio.wait_for(
            _run_download_import(from_day, to_day, force_login),
