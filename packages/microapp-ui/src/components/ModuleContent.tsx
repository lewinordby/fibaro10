...
function ModuleChartPanel({ chart }: { chart: ModuleChart }) {
  ...
  const actions = metrics.length > 1 || day ? <div className="flex flex-wrap items-center justify-end gap-3">
    ...
    {day ? <div className="flex items-center gap-2">
      ...
      <input aria-label="Dato" className="form-input w-36" type="date" max={osloToday()} value={day.selectedDay} onChange={(event) => go(event.target.value)} />
    </div> : null}
  </div> : undefined;
...
function TableCell({ column, row, config, coreUrl }: { column: string; row: ModuleRow; config: DomainUiConfig; coreUrl: string }) {
  ...
  if (column === "status") {
    ...
    const negative = ["ikke ok", "feil", "alarm", "stopp", "mangler", "offline"].some((part) => normalized.includes(part));
    const warning = ["varsel", "warning", "forsinket", "ukjent"].some((part) => normalized.includes(part));
    const positive = !negative && !warning && ["ok", "active", "aktiv", "på", "åpen", "ferdig", "success", "vellykket"].some((part) => normalized === part || normalized.startsWith(`${part} `) || normalized.startsWith(`${part} ·`));
    ...
    return <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-semibold ${classes}`}>{label}</span>;
  }
  if (column === "severity" || column === "priority") {
    const label = displayCell(column, value);
    const normalized = String(value || "").toLowerCase();
    const classes = /kritisk|critical|høy|high/.test(normalized)
      ? "bg-red-500/15 text-red-700 dark:text-red-300"
      : /medium|middels/.test(normalized)
        ? "bg-yellow-500/20 text-yellow-800 dark:text-yellow-300"
        : "bg-gray-100 text-gray-600 dark:bg-gray-700 dark:text-gray-300";
    return <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-semibold ${classes}`}>{label}</span>;
  }
...
function EditDialog({ edit, row, create, close, saved }: { edit: ModuleEditConfig; row: ModuleRow; create: boolean; close: () => void; saved: () => void }) {
  ...
  const titleId = useId();
  useEffect(() => {
    formRef.current?.querySelector<HTMLElement>("input:not([type='hidden']), select, textarea, button")?.focus();
...
function ModuleActions({ actions = [], reload, accent }: { actions?: ModuleAction[]; reload: () => void; accent: Accent }) {
  ...
  return <div className="flex flex-wrap items-center gap-3">{actions.map((action) => <button className={`btn ${action.tone === "primary" ? `${buttonClasses[accent]} text-white` : "border-gray-200 bg-white text-gray-600 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-300"}`} disabled={Boolean(busy)} key={action.key} onClick={() => run(action)}>{busy === action.key ? <MosaicIcon name="refresh" className="animate-spin" /> : null}{action.label}</button>)}{notice ? <span className="text-sm text-gray-600 dark:text-gray-300" role="status" aria-live="polite">{notice}</span> : null}</div>;
}

function DayNavigation({ data }: { data: NonNullable<ModuleResponse["dayNavigation"]> }) {
  const [params, setParams] = useAppSearchParams();
  const go = (day: string) => { const next = new URLSearchParams(params); if (day) next.set("day", day); else next.delete("day"); setParams(next); };
  return <Panel><div className="flex flex-wrap items-center justify-between gap-4 px-5 py-4"><div className="flex items-center gap-2"><button className="btn border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800" aria-label="Forrige dag" onClick={() => go(data.prevDay)}><MosaicIcon name="arrow-left" /></button><button className="btn border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800" onClick={() => go("")}>I dag</button><button className="btn border-gray-200 bg-white dark:border-gray-700 dark:bg-gray-800" aria-label="Neste dag" disabled={data.isToday} onClick={() => go(data.nextDay)}><MosaicIcon name="arrow-right" /></button></div><label className="flex items-center gap-3 text-sm text-gray-500"><strong className="text-gray-800 dark:text-gray-100">{data.selectedDayLabel}</strong><input className="form-input" aria-label="Dato" type="date" max={osloToday()} value={data.selectedDay} onChange={(event) => go(event.target.value)} /></label></div></Panel>;
}
