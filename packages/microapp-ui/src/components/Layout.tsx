...
export function Layout({ config, children }: { config: DomainUiConfig; children: ReactNode }) {
  ...
  const item = useMemo<NavigationItem>(() => config.navigation.flatMap((group) => group.items).find((entry) => entry.to === location.pathname) || config.navigation[0].items[0], [config, location.pathname]);
  return <div className="flex h-[100dvh] overflow-hidden"><Sidebar config={config} open={sidebarOpen} setOpen={setSidebarOpen} shellUrl={appConfig.data?.shellAppUrl || "http://192.168.20.218:8150"} coreUrl={appConfig.data?.fibaro10AppUrl || "http://192.168.20.218:8110"} build={appConfig.data?.build || "-"} /><div className="relative flex flex-1 flex-col overflow-x-hidden overflow-y-auto"><Header config={config} open={sidebarOpen} setOpen={setSidebarOpen} username={user.data?.username || "Bruker"} /><main className="grow"><div className="mx-auto w-full max-w-[96rem] px-4 py-8 sm:px-6 lg:px-8"><div className="mb-7"><h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100 md:text-3xl">{item.title}</h1><p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{item.description}</p></div>{children}</div></main></div></div>;
}
